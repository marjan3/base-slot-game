...
import '@mtanevski/slot-ui/dist/slot-ui.css';
...
export default class LoadGameStateHandler implements GameStateHandler {
  ...
  public async handle() {
    ...
    const gameUi = new SlotUi.GameUi(config).$mount(
      '#slotGameEngineMountPoint'
    );
    window.Game.ui = gameUi;

    // create PIXI application
    gameUi.observable.emit('updateLoading', {
    ...
    gameUi.observable.on('loadingEnded', () => {
      ...
      const game: HTMLElement = document.getElementById(
        'yet-another-slot-game'
      ) as HTMLElement;
